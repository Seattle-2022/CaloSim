...
parser.add_argument('--mode', '-m', default='all',
                    choices=['all', 'avg', 'avg-E', 'hist-p', 'hist-chi', 'hist',
                             'cls-low', 'cls-low-normed', 'cls-high', 'cls-cnn', 'fpd', 'kpd'],
                    help=("What metric to evaluate: " +\
                          ...
                          "'cls-cnn' trains a convolutional classifier;" +\
...
parser.add_argument('--binning_dataset', help = 'Name and path of the xml binning file for particle.')
...
parser.add_argument('--cls_cnn_channels', type=int, default='32',
                    help='Channels of the cnn classifier, default is 32')
...
class CNN(torch.nn.Module):
  """ NN for convolutional classifier. Does not have sigmoid activation in last layer, should
        be used with torch.nn.BCEWithLogitsLoss()
  """

  def __init__(self, HLF, num_channels = 32, dropout_probability=0.):
    super(CNN, self).__init__()
    
    self.num_channels = num_channels
    self.dpo = dropout_probability
    self.num_alphas = HLF.num_alpha
    self.num_radii = HLF.r_edges
    self.relevant_layers = HLF.relevantLayers

    self.cnn0 = torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size = [3,1], stride=1,padding = 0)
    self.lin0 = torch.nn.Sequential(*[torch.nn.Linear(32*8, 64), 
                            torch.nn.LeakyReLU(),
                            torch.nn.Dropout(self.dpo)])

    self.cnn1 = torch.nn.Sequential(*[torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size = [3,4], stride=1, padding = 0), 
            torch.nn.MaxPool2d(kernel_size = [2,2], padding = 1)])
            
    self.lin1 = torch.nn.Sequential(*[torch.nn.Linear(32*9*5, 64), 
            torch.nn.LeakyReLU(),
            torch.nn.Dropout(self.dpo)])


    self.cnn2 = torch.nn.Sequential(*[torch.nn.Conv2d(in_channels=1, out_channels=32, kernel_size
            = [3,4], stride=1, padding = 0),
            torch.nn.MaxPool2d(kernel_size = [2,2], padding = 1)])
            
    self.lin2 = torch.nn.Sequential(*[torch.nn.Linear(32*10*5, 64),
            torch.nn.LeakyReLU(),
            torch.nn.Dropout(self.dpo)])
    ...
    self.lin4 = torch.nn.Sequential(*[
            torch.nn.Linear(5, 64),
            torch.nn.LeakyReLU()])
            
    self.cnn_final = torch.nn.Sequential(*[torch.nn.Conv2d(in_channels=1, out_channels=16, kernel_size = [3,5], stride=1,         padding = 2),
        torch.nn.MaxPool2d(kernel_size = [1,5]),
        torch.nn.Conv2d(in_channels=16, out_channels=32, kernel_size = [3,5], stride=1, padding = 1),
        torch.nn.MaxPool2d(kernel_size = [2,5])])
        
    self.lin_final = torch.nn.Sequential(*[torch.nn.Linear(1 + 32*3*2, 2048),
        torch.nn.LeakyReLU(),
        torch.nn.Dropout(self.dpo),
  ...
  def forward(self, x):
    ...
    layer = torch.reshape(x[:, :self.num_alphas[idx]*(len(self.num_radii[idx]) - 1)], (x.size(0), self.num_alphas[idx],-1)).permute(0, 2, 1)
    
    
    x = x[:, self.num_alphas[idx]*(len(self.num_radii[idx]) - 1):]
    
    input = pad_circular(layer,1, bins = 1).reshape(x.size(0),1,10,1)

    output = self.cnn0(input)

    fcn = output.view(output.size(0), -1)

    output = self.lin0(fcn)
    ...
    layer = torch.reshape(x[:, :self.num_alphas[idx]*(len(self.num_radii[idx]) - 1)], (x.size(0), self.num_alphas[idx],-1)).permute(0, 2, 1)
    x = x[:, self.num_alphas[idx]*(len(self.num_radii[idx]) - 1):]
    input = pad_circular(layer,1).reshape(x.size(0),1,18,12)
    output = self.cnn1(input)
    fcn = output.view(output.size(0), -1)
    output = self.lin1(fcn)
    ...
    input = pad_circular(layer,1).reshape(x.size(0),1,21,12)
    output = self.cnn2(input)
    fcn = output.view(output.size(0), -1)
    output = self.lin2(fcn)
    ...
    output = self.lin3(layer.reshape(x.size(0),5))
    output_layers.append(output)
    ...
    output = self.lin4(layer.reshape(x.size(0),5))
    output_layers.append(output)


    # combine layers
    output = torch.stack(output_layers, 1).reshape(output.size(0),1,5,64)
    output = self.cnn_final(output)
    
    fcn = output.view(output.size(0), -1)
    
    fcn = torch.cat((energy, fcn), dim = 1)
    output = self.lin_final(fcn)
...
def pad_circular(x, pad, bins = 0):
    ...
    x = torch.concatenate([x, x[:, 0:pad]], axis=1)
    if bins != 1:
      x = torch.concatenate([x, x[:, :, 0:pad]], axis=2)
    x = torch.concatenate([x[:, -2 * pad:-pad], x], axis=1)

    if bins != 1:
      x = torch.concatenate([x[:, :, -2 * pad:-pad], x], axis=2)
...
def extract_shower_and_energy(given_file, which):
    ...
    print("Extracting energy from {} file: DONE.\n".format(which))
...
if __name__ == '__main__':
    ...
    if args.mode in ['all', 'cls-cnn']:
        ...
        # set up CNN classifier
        input_dim = train_data.shape[1]-1
        CNN_kwargs = {'HLF': hlf,
                      'num_channels':args.cls_cnn_channels,
                      'dropout_probability':args.cls_dropout_probability}
        classifier = CNN(**CNN_kwargs)
        classifier.to(args.device)
        print(classifier)
    ...
    if args.mode in ['all', 'fpd', 'kpd']:
        ...
        result_str = (
            f"FPD (x10^3): {fpd_val*1e3:.4f} Ãƒâ€šÃ‚Â± {fpd_err*1e3:.4f}\n" 
            f"KPD (x10^3): {kpd_val*1e3:.4f} Ãƒâ€šÃ‚Â± {kpd_err*1e3:.4f}"
        )

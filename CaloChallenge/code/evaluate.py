...
def train_cls(model, data_train, optim, epoch, arg):
    ...
    for i, data_batch in enumerate(data_train):
        if arg.save_mem:
            data_batch = data_batch[0].to(arg.device).float()
        else:
...
def calibrate_classifier(model, calibration_data, arg):
    ...
    for j, data_batch in enumerate(calibration_data):
        ...
        target = target_vector.to(torch.float32)
        if j == 0:
            result_true = target
            result_pred = pred
        else:
...
if __name__ == '__main__':
    ...
    if args.mode in ['all', 'cls-low', 'cls-high', 'cls-low-normed']:
        ...
        if args.save_mem:
            ...
            train_data = TensorDataset(torch.tensor(train_data).to(args.device).float())
            test_data = TensorDataset(torch.tensor(test_data).to(args.device).float())
            val_data = TensorDataset(torch.tensor(val_data).to(args.device).float())
